defmodule Broadway.CallerAcknowledger do
  ...
  Use this acknowledger in messages produced by your
  `Broadway.Producer` to get its configuration by calling
  the `init/0` function. For example, use it in
  ...
  The first parameter is a 2-tuple, `{pid, ref}`. The PID is the process that
  receives acknowledgment messages, and `ref` is a unique identifier (prefer `make_ref()`).
  That unique identifier is then included in the messages sent to the PID. The second parameter,
